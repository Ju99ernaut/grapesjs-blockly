...
export function walk1(ast, comments, block_loc, options) {
    ...
    funcs.IfStatement = (node, st, c) => {
        ...
        current_node = value1;
        c(node.test, st, "Expression")
        var statement1 = newNode('statement', {
            name: 'DO0'
        });
        ...
        c(node.consequent, st, "Statement")
        if (node.alternate) {
            var statement2 = newNode('statement', {
                name: 'ELSE'
            });
    ...
    funcs.SwitchStatement = (node, st, c) => {
        ...
        for (let i = 0; i < node.cases.length - hasDefault; ++i) {
            let item = newNode('value', {
                name: 'items' + (i + 1)
            })
            block1.appendChild(item)
            let case1 = newNode('block', {
                type: 'bi_case'
            }, '', node)
            item.appendChild(case1)
            let test1 = newNode('value', {
                name: 'case'
            })
        ...
        if (hasDefault) {
            let default1 = newNode('statement', {
                name: 'default'
            })
    ...
    funcs.TryStatement = (node, st, c) => {
        ...
        c(node.block, st, "Statement")
        if (node.handler) {
            let param1 = newNode('field', {
                name: 'parameter'
            }, node.handler.param.name)
            block1.appendChild(param1)
            let catch1 = newNode('statement', {
                name: 'catch'
            })
        ...
        if (node.finalizer) {
            let finally1 = newNode('statement', {
                name: 'finally'
            })
    ...
    funcs.WhileStatement = funcs.DoWhileStatement = (node, st, c) => {
        ...
        current_node.appendChild(block1);
        var mode1 = newNode('field', {
            name: 'MODE'
        }, 'WHILE')
        block1.appendChild(mode1)
        var test1 = newNode('value', {
            name: 'BOOL'
        })
    ...
    funcs.ForInStatement = funcs.ForOfStatement = (node, st, c) => {
        ...
        current_node.appendChild(block1);
        var init1 = newNode('field', {
            name: 'var'
        }, node.left.declarations[0].id.name); // Only one declaration allowed.
        block1.appendChild(init1);
        //c(node.left, st, "ForInit") // We are setting the variable here.
        var array1 = newNode('value', {
            name: 'array'
        });
    ...
    funcs.VariableDeclarator = (node, st, c) => {
        ...
        if (!first_variable_declarator) {
            ...
            if (for_init) {
                let block1 = newNode('block', {
                    type: 'bi_assignment'
                }, '', node);
                current_node.appendChild(block1);
                var field1 = newNode('field', {
                    name: 'OP'
                }, '=');
                block1.appendChild(field1);
                var left = newNode('value', {
                    name: 'A'
                });
                ...
                block1.appendChild(newNode('field', {
                    name: 'var_type'
                }, node.kind));
                block1.appendChild(newNode('field', {
                    name: 'var'
                }, node.id.name));
                let value1 = newNode('value', {
                    name: 'val'
                });
                block1.appendChild(value1);
                current_node.appendChild(block1);
    ...
    funcs.ArrayExpression = (node, st, c) => {
        ...
        current_node.appendChild(block1);
        var mutation1 = newNode('mutation', {
            items: node.elements.length + 1
        }); // TODO: Take out the +1 when list items number is corrected on core/blocks.js AddSub...
        block1.appendChild(mutation1);
        var node1 = current_node;
        for (let i = 0; i < node.elements.length; ++i) {
            let element1 = newNode('value', {
                name: 'items' + (i + 1)
            });
    ...
    funcs.ObjectExpression = (node, st, c) => {
        ...
        current_node.appendChild(block1);
        var mutation1 = newNode('mutation', {
            items: node.properties.length + 1
        }); // TODO: Take out the +1 when list items number is corrected on core/blocks.js AddSub...
        block1.appendChild(mutation1);
        var node1 = current_node;
        for (let i = 0; i < node.properties.length; ++i) {
            let property1 = newNode('value', {
                name: 'ADD' + (i + 1)
            });
    ...
    funcs.NewExpression = (node, st, c) => {
        ...
        call1.appendChild(newNode('field', {
            name: 'NAME'
        }, node.callee.name));
        c(node.callee, st, "Expression")
    ...
    funcs.ImportDeclaration = (node, st, c) => {
        ...
        var node1 = current_node;
        var mutation1 = newNode('mutation', {
            items: node.specifiers.length + 1
        }) // TODO: Take out +1 when items count corrected on AddSub block
        block1.appendChild(mutation1)
        for (let i = 0; i < node.specifiers.length; i++) {
            let item = newNode('value', {
    ...
    funcs.MethodDefinition = (node, st, c) => {
        ...
        var statement1
        if (node.static) {
            let static1 = newNode('block', {
                type: 'bi_static'
            }, '', node)
            current_node.appendChild(static1)
            let statement2 = newNode('statement', {
                name: 'static'
            })
    ...
    funcs.Property = (node, st, c) => {
        ...
        switch (node.kind) {
            case 'set':
                ...
                block1.appendChild(newNode('field', {
                    name: 'val'
                }, node.value.params[0].name));
                chain1 = newNode('statement', {
                    name: 'chain'
                });
    ...
    if (last_comment < comments.length) {
        current_node.appendChild(newNode('block', {
            type: 'bi_comment'
        }, '', fake_last_line_node))
    }
